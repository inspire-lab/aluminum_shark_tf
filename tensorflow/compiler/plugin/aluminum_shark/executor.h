...
#ifndef TENSORFLOW_COMPILER_PLUGIN_ALUMINUM_SHARK_EXECUTOR_H
#define TENSORFLOW_COMPILER_PLUGIN_ALUMINUM_SHARK_EXECUTOR_H
...
class XlaAluminumSharkExecutor : public internal::StreamExecutorInterface {
 public:
  explicit XlaAluminumSharkExecutor(const PluginConfig &plugin_config);
  ~XlaAluminumSharkExecutor() override;
  ...
  port::Status MemZero(Stream *stream, DeviceMemoryBase *location,
                       uint64 size) override {
    return port::InternalError("AluminumShark can not memzero");
  }
  port::Status Memset(Stream *stream, DeviceMemoryBase *location, uint8 pattern,
                      uint64 size) override {
    return port::InternalError("AluminumShark can not memset");
  }
...
#endif /*TENSORFLOW_COMPILER_PLUGIN_ALUMINUM_SHARK_EXECUTOR_H */
