...
class Ctxt : public BaseTxt {
 public:
  // Ctor/Dtor
  Ctxt() {}
  Ctxt(std::string name) : name_(name) {}
  Ctxt(std::shared_ptr<HECtxt> hectxt, std::string name);
  // Takes over the owernship of cxt_ptr
  Ctxt(HECtxt* ctxt_ptr, std::string name)
      : Ctxt(std::shared_ptr<HECtxt>(ctxt_ptr), name) {}

  virtual ~Ctxt() {}
  // getters / setters
  const HECtxt& getValue() const;
  HECtxt& getValue();
  std::shared_ptr<HECtxt> getValuePtr() const;
  void setValue(std::shared_ptr<HECtxt> value_ptr);
