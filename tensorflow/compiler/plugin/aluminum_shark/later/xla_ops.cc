...
auto* xla_launch_counter = tf::monitoring::Counter<1>::New(
...
class XlaExecutableClosure {
 public:
  explicit XlaExecutableClosure(
      ...
      const tf::XlaCompiler::CompilationResult* compilation_result,
      tf::ResourceVarsSnapshot resource_var_snapshots, int num_constant_args)
      : client_(client),
  ...
  const tf::XlaCompiler::CompilationResult* compilation_result() const {
    return compilation_result_;
  }
  const tf::ResourceVarsSnapshot& resource_var_snapshots() const {
 ...
 private:
  ...
  const tf::XlaCompiler::CompilationResult* compilation_result_;
  tf::ResourceVarsSnapshot resource_var_snapshots_;
...
XlaLocalLaunchBase::XlaLocalLaunchBase(tf::OpKernelConstruction* ctx,
                                       const std::vector<int>& constants,
                                       const std::vector<int>& resources,
                                       const tf::NameAttrList& function,
                                       bool has_ref_vars)
    : OpKernel(ctx),
...
static tf::Status CompileToLocalExecutable(
    tf::OpKernelContext* ctx, const tf::NameAttrList& function,
    bool has_ref_vars, const tf::XlaPlatformInfo& platform_info,
    absl::Span<const tf::Tensor* const> inputs,
    absl::Span<tf::VariableInfo const> variable_infos,
    absl::Span<const int> constants,
    tf::XlaCompilationCache::CompileMode compile_mode,
    bool may_alias_resource_update, xla::LocalClient** client,
    const tf::XlaCompiler::CompilationResult** compilation_result,
    xla::LocalExecutable** executable) {
  // We store information about the JIT-compiled XLA computation
  // in the ResourceMgr.
  tf::ResourceMgr* rm = ctx->resource_manager();
  if (!rm) {
    return tf::errors::Internal("No resource manager.");
  }

  tf::XlaCompilationCache* cache;
  TF_RETURN_IF_ERROR(rm->LookupOrCreate<tf::XlaCompilationCache>(
      rm->default_container(), "xla_cache", &cache,
      [&](tf::XlaCompilationCache** cache) {
        return tf::BuildXlaCompilationCache(
            ctx->device(), ctx->function_library(), platform_info, cache);
      }));
  // Hold the reference to the JIT during evaluation. (We could probably
  // free it sooner because the ResourceMgr will retain a reference, but
  // this is more obviously correct.)
  tf::core::ScopedUnref cache_ref(cache);

  *client = static_cast<xla::LocalClient*>(cache->client());

  tf::XlaCompiler::Options options = GenerateCompilerOptions(
      *cache, *ctx->function_library(), ctx->device(),
      ctx->op_device_context() ? ctx->op_device_context()->stream() : nullptr,
      platform_info, has_ref_vars);

  tf::XlaCompiler::CompileOptions compile_options;
  ...
  tf::StatusOr<std::vector<tf::XlaCompiler::Argument>> args =
      tf::XlaComputationLaunchContext::BuildXlaCompilerArguments(
          constants, inputs, variable_infos,
          static_cast<tf::Device*>(ctx->device()));
  TF_RETURN_IF_ERROR(args.status());
  return cache->Compile(options, function, *args, compile_options, compile_mode,
                        compilation_result, executable);
}

void XlaLocalLaunchBase::Compute(tf::OpKernelContext* ctx) {
  VLOG(1) << "XlaLocalLaunchOpBase::Compute "
          << tf::Canonicalize(function_.name(),
                              tf::AttrSlice(&function_.attr()));
  xla_launch_counter->GetCell(platform_info_.device_type().type_string())
      ->IncrementBy(1);

  std::vector<const tf::Tensor*> inputs = InputsFromContext(ctx);
  xla::LocalClient* client;
  const tf::XlaCompiler::CompilationResult* compilation_result;
  xla::LocalExecutable* executable;

  std::vector<tf::VariableInfo> variable_infos;
  {
    OP_REQUIRES_OK(ctx, tf::GetVariableInfosFromInputs(
                            ctx->resource_manager(), ctx->device(), inputs,
                            resources_, &variable_infos));
    OP_REQUIRES_OK(ctx, tf::LockVariables(absl::MakeSpan(variable_infos)));
    tf::Status s = CompileToLocalExecutable(
        ctx, function_, /*has_ref_vars=*/has_ref_vars_, platform_info_, inputs,
        variable_infos, constants_,
        tf::XlaCompilationCache::CompileMode::kStrict,
        /*may_alias_resource_update=*/true, &client, &compilation_result,
        &executable);
    OP_REQUIRES_OK(ctx, s);
  }

  std::map<int, const tf::Tensor*> resource_var_ptrs;
  ...
  stream_executor::Stream* stream =
      ctx->op_device_context() ? ctx->op_device_context()->stream() : nullptr;
  std::shared_ptr<stream_executor::DeviceMemoryAllocator> allocator_ptr =
      GetAllocator(ctx->device(), stream, platform_info_);
  stream_executor::DeviceMemoryAllocator* allocator = allocator_ptr.get();
  int device_ordinal = stream ? stream->parent()->device_ordinal()
                              : client->default_device_ordinal();
  tf::XlaComputationLaunchContext launch_context(
  ...
  tf::StatusOr<std::vector<xla::ExecutionInput>> execution_inputs =
  ...
  run_options.set_rng_seed(tf::GetXLARandomSeed());
  ...
  tf::StatusOr<xla::ExecutionOutput> execution_output;
  if (!stream ||
      platform_info_.platform_id() == stream_executor::host::kHostPlatformId) {
    execution_output =
...
std::vector<int> ConstantsVector(tf::OpKernelConstruction* ctx) {
  tf::DataTypeVector constant_types;
...
std::vector<int> ResourcesVector(tf::OpKernelConstruction* ctx) {
  tf::DataTypeVector constant_types;
  OP_REQUIRES_OK_RETURN(ctx, std::vector<int>(),
                        ctx->GetAttr("Tconstants", &constant_types));

  tf::DataTypeVector arg_types;
  OP_REQUIRES_OK_RETURN(ctx, std::vector<int>(),
...
tf::NameAttrList FunctionAttr(tf::OpKernelConstruction* ctx) {
  const tf::NameAttrList* func;
  OP_REQUIRES_OK_RETURN(ctx, tf::NameAttrList(),
                        ctx->GetAttr("function", &func));
  return *func;
}

bool MustCompileAttr(tf::OpKernelConstruction* ctx) {
  bool must_compile;
...
bool HasRefVars(tf::OpKernelConstruction* ctx) {
  bool has_ref_vars;
  OP_REQUIRES_OK_RETURN(
      ctx, false, ctx->GetAttr(tf::kXlaHasReferenceVarsAttr, &has_ref_vars));
  return has_ref_vars;
}

}  // namespace

XlaLocalLaunchOp::XlaLocalLaunchOp(tf::OpKernelConstruction* ctx)
...
XlaCompileOp::XlaCompileOp(tf::OpKernelConstruction* ctx)
    : tf::OpKernel(ctx),
...
void XlaCompileOp::Compute(tf::OpKernelContext* ctx) {
  VLOG(3) << "XlaCompileOp " << def().name()
          << (must_compile_ ? "(must-compile)" : "");
  xla::LocalClient* client;
  const tf::XlaCompiler::CompilationResult* kernel;
  xla::LocalExecutable* executable;
  tf::ResourceVarsSnapshot variables;

  std::vector<const tf::Tensor*> inputs = tf::InputsFromContext(ctx);
  bool cannot_compile_cluster;
  {
    tf::mutex_lock guard(cannot_compile_cluster_mu_);
    cannot_compile_cluster = cannot_compile_cluster_;
  }
  tf::XlaCompilationCache::CompileMode compile_mode = [&] {
    if (must_compile_) {
      return tf::XlaCompilationCache::CompileMode::kStrict;
    }
    return tf::GetXlaOpsCommonFlags().tf_xla_async_compilation
               ? tf::XlaCompilationCache::CompileMode::kAsync
               : tf::XlaCompilationCache::CompileMode::kLazy;
  }();

  if (tf::GetXlaOpsCommonFlags().tf_xla_always_defer_compilation ||
    ...
    tf::Status status = CompileToLocalExecutable(
    ...
    if (compile_mode != tf::XlaCompilationCache::CompileMode::kLazy ||
        status.code() != tf::error::UNIMPLEMENTED) {
      OP_REQUIRES_OK(ctx, status);
    }

    if (status.code() == tf::error::UNIMPLEMENTED) {
      LOG(WARNING) << "Compilation failed:" << status.ToString()
                   << ".  Falling back to TF function call.";

      tf::BroadcastOptimizationRemark(
          tf::XlaOptimizationRemark::UNIMPLEMENTED_OPERATION, status.ToString())
          .IgnoreError();
      executable = nullptr;
      tf::mutex_lock guard(cannot_compile_cluster_mu_);
  ...
  tf::AllocatorAttributes host_alloc_attrs;
  host_alloc_attrs.set_gpu_compatible(true);
  host_alloc_attrs.set_on_host(true);
  tf::Allocator* cpu_allocator = ctx->device()->GetAllocator(host_alloc_attrs);

  // Async compilation returns nullptr executable without an error.
  if (!executable) {
    DCHECK(!must_compile_);
    tf::Tensor compilation_key(cpu_allocator, tf::DT_STRING,
                               tf::TensorShape({}));

    tf::Tensor compilation_successful(cpu_allocator, tf::DT_BOOL,
                                      tf::TensorShape({}));
    compilation_successful.scalar<bool>()() = false;
    ctx->set_output(
        0, tf::Tensor(cpu_allocator, tf::DT_STRING, tf::TensorShape({})));
  ...
  tf::Tensor compilation_key(cpu_allocator, tf::DT_STRING, tf::TensorShape({}));
  compilation_key.flat<tf::tstring>()(0) = key;
...
XlaRunOp::XlaRunOp(tf::OpKernelConstruction* ctx)
    : OpKernel(ctx), platform_info_(XlaPlatformInfoFromDevice(ctx->device())) {}

void XlaRunOp::Compute(tf::OpKernelContext* ctx) {
  VLOG(3) << "XlaRunOp " << def().name();
  tf::Tensor key_tensor = ctx->input(ctx->num_inputs() - 1);
  const XlaExecutableClosureStore::KeyT& key =
      key_tensor.flat<tf::tstring>()(0);
  ...
  tf::StatusOr<std::vector<xla::ExecutionInput>> execution_inputs;
  std::map<int, const tf::Tensor*> snapshot_ptrs;
  {
  ...
  tf::StatusOr<std::vector<tf::VariableInfo>> variable_infos =
      tf::GatherVariableInfo(ctx, *closure.compilation_result(),
...
XlaMergeOp::XlaMergeOp(tf::OpKernelConstruction* ctx) : OpKernel(ctx) {}

void XlaMergeOp::Compute(tf::OpKernelContext* ctx) {
...
// REGISTER_KERNEL_BUILDER(Name("XlaLaunch").Device(DEVICE_XLA_HE),
//                         XlaLocalLaunchOp);

// REGISTER_KERNEL_BUILDER(Name("XlaLaunch")
//                             .Device(DEVICE_XLA_HE)
//                             .HostMemory("constants")
//                             .HostMemory("resources"),
//                         XlaLocalLaunchOp);

// REGISTER_KERNEL_BUILDER(Name("_XlaCompile").Device(DEVICE_XLA_HE),
//                         XlaCompileOp);
...
// REGISTER_KERNEL_BUILDER(Name("_XlaRun").Device(DEVICE_XLA_HE), XlaRunOp);
// REGISTER_KERNEL_BUILDER(Name("_XlaRun").Device(DEVICE_XLA_HE).HostMemory("key"),
//                         XlaRunOp);

// REGISTER_KERNEL_BUILDER(Name("_XlaMerge").Device(DEVICE_XLA_HE), XlaMergeOp);
// REGISTER_KERNEL_BUILDER(Name("_XlaMerge").Device(DEVICE_XLA_HE), XlaMergeOp);
