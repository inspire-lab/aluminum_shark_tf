...
// AluminumSharkHloEvaluatorTypedVisitor so they don't pollute namespace xla,
// but that crashes clang in the frontend.
//
...
// Templated DfsHloVisitor for use by AluminumSharkHloEvaluator.
//
...
// These operations are handled outside of the parent AluminumSharkHloEvaluator
// handlers instead of from within TypedVisitor.
//
// Type params:
//   - ReturnT: The type of input and output of each operation.
//   - ElementwiseT: The type in which internal computation are done.
//
// This is logically a private part of AluminumSharkHloEvaluator.  It lives in
// this header file rather than in hlo_evaluator.cc because we use extern
// templates and a bunch of independent cc files to speed up compiling the many
// instantiations of this class.
template <typename ReturnT, typename ElementwiseT = ReturnT>
class AluminumSharkHloEvaluatorTypedVisitor : public DfsHloVisitorWithDefault {
 private:
 ...
 public:
  explicit AluminumSharkHloEvaluatorTypedVisitor(AluminumSharkHloEvaluator* p)
      : parent_(p) {}
  ...
  Status DefaultAction(HloInstruction* hlo_instruction) override {
    return Unimplemented("unhandled HLO ops for AluminumSharkHloEvaluator: %s.",
  ...
  Status HandleAbs(HloInstruction* abs) {
    ...
    TF_ASSIGN_OR_RETURN(
        ...
        (AluminumSharkHloEvaluator::ElementWiseUnaryOpImpl<
            typename NativeT::value_type, NativeT>(
  ...
  Status HandleConvolutionWithLiterals(HloInstruction* conv,
    ...
    Literal result(result_shape);
    // TODO RP: renable this in case of debug inputs
  ...
  Status HandleDot(HloInstruction* dot) {
    ...
    AS_LOG_INFO << " Handling Dot operation: " << lhs->shape().ToString()
                << " x " << rhs->shape().ToString() << " -> " << dot->ToString()
                << std::endl;
    ...
    if (lhs_rank == 2 && rhs_rank == 2 && lhs_contracting_dimension == 1 &&
      ...
      std::unique_ptr<Array2D<NativeT>> result_array =
          AluminumSharkHloEvaluator::MatmulArray2D(lhs_array, rhs_array);
  ...
  Status HandleDotSlowPathWithLiterals(HloInstruction* dot,
    ...
    Literal result(dot->shape());
    // TODO RP: enable if debug input is present
    if (::aluminum_shark::log(::aluminum_shark::AS_DEBUG)) {
      TF_RETURN_IF_ERROR(
          result.Populate<ReturnT>([&](absl::Span<const int64_t> result_index) {
            ...
            for (int64_t k = 0; k < total_contraction_size; k++) {
              ...
              if (!accumulate_index_sizes.empty()) {
                for (int64_t i = accumulate_index_sizes.size() - 1; i >= 0;
                     --i) {
  ...
  // corresponding function for Gather.
 ...
 private:
  ...
  StatusOr<Literal> ElementWiseUnaryOp(
    ...
    TF_ASSIGN_OR_RETURN(
        auto result_literal,
        (AluminumSharkHloEvaluator::ElementWiseUnaryOpImpl<ReturnT, ReturnT>(
  ...
  ::aluminum_shark::Ctxt* ElementWiseBinaryOpCtxtInplace(
      HloInstruction* instruction,
      const std::function<::aluminum_shark::Ctxt*(
...
extern template class AluminumSharkHloEvaluatorTypedVisitor<bool>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<uint8>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<uint32>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<uint64_t>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<int8>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<int32>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<int64_t>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<Eigen::half, float>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<float>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<double>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<complex64>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<complex128>;
extern template class AluminumSharkHloEvaluatorTypedVisitor<bfloat16, float>;

}  // namespace aluminum_shark
}  // namespace xla

#endif /* ALUMINUM_SHARK_DEPENDENCIES_TENSORFLOW_TENSORFLOW_COMPILER_PLUGIN_ALUMINUM_SHARK_HLO_EVALUATOR_TYPED_VISITOR_H \
        */
